...
def get_population_data():
    ...
    state_pop_data.loc[:, "state"] = state_pop_data["NAME"].map(lambda x: states.lookup(x).abbr)
...
def get_vote_rep(voters):
    ...
    votes.loc[:, "rep"] = votes[CENSUS_POPULATION_CODE] / (int(us_pop_data[0][CENSUS_POPULATION_CODE]) * 2) 
...
def get_party_rep(voters):
    """ Gets the ratio of Yea votes made by the non-majority party to the majority party. """
    votes = voters.loc[lambda x: 
        (x["party"].isin(["D", "R"])) & 
        (x["vote_cast"] == "Yea")
        ]
